...
public class CalcDay implements Runnable {
	@Autowired
	...
	private static final Logger logger = LoggerFactory.getLogger(CalcDay.class);
		...
		try {
			for (long count = 1; count <= resultDao.count(); count++) {
				...
				for (Data d : dataDao.findByPlatform((int) count)) {
					sum += Double.parseDouble(d.getDay());
				}

				try {
					averange = sum / dataDao.count();
					this.Day[(int) count] = averange;
				} catch (Exception e) {
