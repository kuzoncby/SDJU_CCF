...
public class CalcReward implements Runnable {
	...
	private static final Logger logger = LoggerFactory.getLogger(CalcReward.class);
		...
		try {
			for (long count = 1; count <= resultDao.count(); count++) {
				...
				for (Data d : dataDao.findByPlatform((int) count)) {
					sum += Double.parseDouble(d.getReward());
				}

				try {
					averange = sum / dataDao.count();
					this.Reward[(int)count] = averange;
				} catch (Exception e) {
