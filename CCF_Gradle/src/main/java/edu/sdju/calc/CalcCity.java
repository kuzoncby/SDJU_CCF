...
public class CalcCity implements Runnable {
	@Autowired
	...
	private static final Logger logger = LoggerFactory.getLogger(CalcCity.class);
		...
		try {
			for (long count = 1; count <= resultDao.count(); count++) {
				...
				for (Data d : dataDao.findByPlatform((int) count)) {
					sum += Double.parseDouble(d.getCity());
				}

				try {
					averange = sum / dataDao.count();
					this.City[(int)count] = averange;
				} catch (Exception e) {
