...
public class CalcMarriage implements Runnable {
	...
	private static final Logger logger = LoggerFactory.getLogger(CalcMarriage.class);

	public double[] Marriage;
		...
		try {
			for (long count = 1; count <= resultDao.count(); count++) {
				...
				for (Data d : dataDao.findByPlatform((int) count)) {
					sum += Double.parseDouble(d.getMarriage());
				}

				try {
					averange = sum / dataDao.count();
					this.Marriage[(int)count] = averange;
				} catch (Exception e) {
