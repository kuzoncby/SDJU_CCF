...
public class CalcCar implements Runnable{
	...
	private static final Logger logger = LoggerFactory.getLogger(CalcCar.class);
		...
		try {
			for (long count = 1; count <= resultDao.count(); count++) {
				...
				for (Data d : dataDao.findByPlatform((int) count)) {
					sum += Double.parseDouble(d.getCar());
				}

				try {
					averange = sum / dataDao.count();
					this.Car[(int)count] = averange;
				} catch (Exception e) {
