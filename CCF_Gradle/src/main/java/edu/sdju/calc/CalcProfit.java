...
public class CalcProfit implements Runnable {
	...
	private static final Logger logger = LoggerFactory.getLogger(CalcProfit.class);
		...
		try {
			for (long count = 1; count <= resultDao.count(); count++) {
				...
				for (Data d : dataDao.findByPlatform((int) count)) {
					sum += Double.parseDouble(d.getProfit());
				}

				try {
					averange = sum / dataDao.count();
					this.Profit[(int)count] = averange;
				} catch (Exception e) {
