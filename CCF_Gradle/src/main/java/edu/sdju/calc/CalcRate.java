...
public class CalcRate implements Runnable{
	@Autowired
	...
	private static final Logger logger = LoggerFactory.getLogger(CalcRate.class);
		...
		try {
			for (long count = 1; count <= resultDao.count(); count++) {
				...
				for (Data d : dataDao.findByPlatform((int) count)) {
					sum += Double.parseDouble(d.getRate());
				}

				try {
					averange = sum / dataDao.count();
					this.Rate[(int)count] = averange;
				} catch (Exception e) {
