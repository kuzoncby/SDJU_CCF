...
public class CalcEducation implements Runnable {
	...
	private static final Logger logger = LoggerFactory.getLogger(CalcEducation.class);
		...
		try {
			for (long count = 1; count <= resultDao.count(); count++) {
				...
				for (Data d : dataDao.findByPlatform((int) count)) {
					sum += Double.parseDouble(d.getEducation());
				}

				try {
					averange = sum / dataDao.count();
					this.Education[(int) count] = averange;
				} catch (Exception e) {
